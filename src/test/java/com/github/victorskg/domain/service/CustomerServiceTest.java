...
class CustomerServiceTest {
    ...
    void shouldCreateCustomer() {
        ...
        final var customer = Customer.builder().name("Customer").phone("999999999999").build();
    ...
    void shouldGetCustomer() {
        final var customer = Customer.builder().uuid(UUID.randomUUID()).name("Customer").phone("999999999999").build();
        Mockito.when(gateway.findByUuid(any(UUID.class))).thenReturn(Optional.of(customer));
